  ...
  let bombInterval = setInterval(() => {
    ...
    bombObj.addEventListener("click", function () {
      ...
      allBirdImages.forEach((element) => {
        ...
        if (
          bombObj.offsetLeft > element.offsetLeft - 200 &&
          element.offsetLeft + 200 > bombObj.offsetLeft &&
          bombObj.offsetTop > element.offsetTop - 200 &&
          element.offsetTop + 200 > bombObj.offsetTop
        ) {
          birdHit(element);
        }
      });
...
const birdHit = function (birdImage) {
  switch (
    birdImage.value //score mangement
  ) {
    case 1:
      score += 5;
      break;
...
const birdMovement = function (birdImage) {
  ...
  let birdInterval = setInterval(() => {
    ...
    if (birdImage.offsetLeft >= "1200") {
      //removing img after reaching the max width.
      clearInterval(birdInterval);
      birdImage.remove();
    }
  }, 100);
